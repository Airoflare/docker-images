...
> [!WARNING]
> **This open-source `mc` is frozen — it is no longer developed.** MinIO stopped releasing the open-source `mc`; the current client is the **closed-source** AIStor build. Here, only the **Go toolchain and base image are refreshed** on each weekly rebuild (so the binary keeps getting Go-runtime and CA/BusyBox security fixes) — the `mc` version, features, and any upstream bug fixes stay pinned at the last open-source release. Use it as a stable drop-in for the removed `minio/mc` image, not as a source of new `mc` functionality. The current client is the closed-source AIStor build (`quay.io/minio/aistor/mc`), which this project does not repackage — its proprietary license does not permit redistribution.
